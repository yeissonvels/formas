...
<script>
    ...
    function check_new_order() {
        comprobate = Array('#code', '#customer', '#telephone', '#purchasedate', '#deliveryrange', '#deliveryzone',
            '#deliverymonth', '#total', '#pendingpay', '#paymethod', "#status");
    ...
    function saveProducts() {
        ...
        if (orderid != 0 && orderid != "") {
            var selectproducts = $('#dynamic-table select');
            var cont = 0;
...
<?php
    ...
    if (!$data['data']) { // No hemos creado el pedido aún
        $canEdit = true;
    } else if ($data['data'] && $data['data']->getStatus() < 2) {
    ...
    <div class="card col-lg-6">
        <div class="card-header">
            <h4 class="card-title">
                <?php
                    if ($canEdit) {
                        echo ($data['data'] ? "Modificar pedido " : "Nuevo pedido "); icon('info', true);
                    } else {
